...
public class FoodItem extends Item {
    ...
    public FoodItem(Settings settings, boolean hasFoodEffectTooltip) {
        super(settings);
        this.hasFoodEffectTooltip = hasFoodEffectTooltip;
        this.hasCustomTooltip = false;
    }

    public FoodItem(Settings settings, boolean hasFoodEffectTooltip, boolean hasCustomTooltip) {
        super(settings);
        this.hasFoodEffectTooltip = hasFoodEffectTooltip;
    ...
    public static void addFoodEffectTooltip(ItemStack itemIn, List<Text> lores, float durationFactor) {
        ...
        if (!attributeList.isEmpty()) {
            ...
            for (Pair<EntityAttribute, EntityAttributeModifier> pair : attributeList) {
                ...
                if (amount > 0.0D) {
                    lores.add((Text.translatable("attribute.modifier.plus." + modifier.getOperation().getId(),
                            ItemStack.MODIFIER_FORMAT.format(formattedAmount), Text.translatable(pair.getFirst().getTranslationKey()))).formatted(Formatting.BLUE));
                } else if (amount < 0.0D) {
                    formattedAmount = formattedAmount * -1.d;
                    lores.add((Text.translatable("attribute.modifier.take." + modifier.getOperation().getId(),
                            ItemStack.MODIFIER_FORMAT.format(formattedAmount), Text.translatable(pair.getFirst().getTranslationKey()))).formatted(Formatting.RED));
                }
            }
        }
    }
